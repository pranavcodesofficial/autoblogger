    ...
    for attempt in range(3):  # Retry up to 3 times
        try:
            ...
            if 'choices' in res_json:
                blog_title = prompt.split(" on ")[-1].replace(".", "").title()
                blog_content = res_json['choices'][0]['message']['content']
                thumbnail = get_unsplash_image("technology AI")
                section_img = get_unsplash_image("hiring AI")
...
if __name__ == "__main__":
    prompt = "Write a 500-word blog on current AI trends in recruitment."
    generate_blog(prompt, "ai_recruitment_blog.html")